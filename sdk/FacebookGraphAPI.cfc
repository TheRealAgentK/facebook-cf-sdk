...
component extends="FacebookBase" {
	 ...
	 */
	public Array function getConnections(required String id, required String type, Numeric limit=-1, Numeric offset=-1, Numeric since, Numeric until, String dateFormat = "") {
		var connections = [];
		...
		if (structKeyExists(arguments, "since")) httpService.addParam(type="url", name="since", value="#arguments.since#");
		if (structKeyExists(arguments, "until")) httpService.addParam(type="url", name="until", value="#arguments.until#");
		result = callAPIService(httpService);
		if (structKeyExists(result, "data")) {
